    ...
    const fetchAll = useCallback(async () => {
        ...
        try {
            ...
            if (viagensRes.status === "fulfilled" && viagensRes.value.ok) {
                try { viagensArr = await viagensRes.value.json(); } catch (e) { viagensArr = []; }
            } else {
            ...
            if (abastecRes.status === "fulfilled" && abastecRes.value.ok) {
                try { abastecArr = await abastecRes.value.json(); } catch (e) { abastecArr = []; }
            } else {
            ...
            if (alertasRes && alertasRes.status === "fulfilled") {
                if (alertasRes.value.ok) {
                    try { alertArr = await alertasRes.value.json(); } catch (e) { alertArr = []; }
                } else {
                        ...
                        <View style={[styles.card, { backgroundColor: theme.card }]}>
                            <View style={styles.cardRow}>
                                <View>
                                    ...
                                    <ThemedText type="title" style={styles.cardNumber}>{stats.solicitacoesManutencaoPendentes}</ThemedText>
                        ...
                        <View style={[styles.card, { backgroundColor: theme.card }]}>
                            <View style={styles.cardRow}>
                                <View>
                                    ...
                                    <ThemedText type="title" style={styles.cardNumber}>{stats.abastecimentosEsteMes}</ThemedText>
                                    <ThemedText style={styles.cardNote}>Este mês</ThemedText>
                                </View>
                                <View style={styles.iconWrap}>
