...
router.get("/", autenticarToken, async (req, res) => {
  try {
    ...
    const take = limit
      ? Math.min(200, parseInt(String(limit), 10) || 200)
      : undefined;
...
router.get("/:id", autenticarToken, async (req, res) => {
  ...
  try {
    ...
    const isAdmin = req.user && req.user.role === "ADMIN";
    if (!isAdmin && a.userId !== req.user.id)
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        error:
          "Campos obrigatórios: veiculoId, data, quilometragem, litros, valorPorLitro, combustivel.",
      });
    }
    ...
    if (isNaN(litrosNum) || isNaN(valorNum))
      return res
        .status(400)
        .json({ error: "litros ou valorPorLitro inválido." });
...
router.patch("/:id", autenticarToken, async (req, res) => {
  ...
  try {
    ...
    if (quilometragem !== undefined)
      updates.quilometragem = Number(quilometragem);
    if (litros !== undefined) updates.litros = Number(litros);
    if (valorPorLitro !== undefined)
      updates.valorPorLitro = Number(valorPorLitro);
    if (litros !== undefined || valorPorLitro !== undefined) {
      const litrosVal =
        litros !== undefined ? Number(litros) : Number(existing.litros);
      const valorVal =
        valorPorLitro !== undefined
          ? Number(valorPorLitro)
          : Number(existing.valorPorLitro);
