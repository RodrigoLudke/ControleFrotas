...
router.post("/", autenticarToken, async (req, res) => {
  ...
  if (!isValidInteger(veiculoId))
    return res.status(400).json({ error: "veiculoId inválido." });
  if (!dataSaida || !dataChegada)
    return res
      .status(400)
      .json({ error: "dataSaida e dataChegada são obrigatórios." });
  if (finalidade === undefined)
    return res.status(400).json({ error: "finalidade é obrigatória." });
  if (!isValidInteger(kmFinal))
    return res.status(400).json({ error: "kmFinal inválido." });
  ...
  if (fimViagem <= inicioViagem)
    return res.status(400).json({
      error: "Data/Horário de chegada não pode ser anterior ou igual à saída.",
    });
  ...
  try {
    ...
    const novaViagem = await prisma.$transaction(async (tx) => {
      ...
      if (ultimaViagem) {
        ...
        if (inicioViagem <= new Date(ultimaViagem.dataChegada)) {
          ...
          throw new Error(
            `A data/hora de saída não pode ser anterior ou igual à chegada da última viagem (${ultimaViagem.dataChegada.toISOString()}).`,
          );
        }
      ...
      if (kmFinalNum <= kmInicialCalculado) {
        throw new Error(
          `A quilometragem final (${kmFinalNum}) não pode ser menor ou igual à quilometragem inicial (${kmInicialCalculado}).`,
        );
      }
    ...
    if (
      error.message.includes("data/hora de saída") ||
      error.message.includes("quilometragem final")
    ) {
...
router.patch("/:id", autenticarToken, async (req, res) => {
  try {
    ...
    if (newDataSaida >= newDataChegada) {
      return res
        .status(400)
        .json({ error: "Data de saída deve ser anterior à data de chegada." });
    }
    ...
    if (kmFinal !== undefined) {
      ...
      if (ultimaOutra && novoKm <= ultimaOutra.kmFinal) {
        return res.status(400).json({
          error:
            "A quilometragem final informada é menor ou igual à última registrada para este veículo.",
