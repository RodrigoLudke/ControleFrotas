  ...
  async (req, res) => {
    try {
      ...
      if (!action || !["accept", "reject"].includes(action)) {
        return res
          .status(400)
          .json({ error: "Ação inválida. Use 'accept' ou 'reject'." });
      }
      ...
      if (agendar) {
        ...
        const statusManut =
          agendar?.status === "CONCLUIDA" ? "CONCLUIDA" : "AGENDADA";
        ...
        const manutData = {
          ...
          descricao:
            descricao ??
            alerta.mensagem ??
            "Manutenção agendada a partir de alerta",
      ...
      return res.json({
        message: "Alerta aprovado.",
        alerta: updatedAlerta,
        manutencao: manutencaoCriada,
      });
    } catch (err) {
