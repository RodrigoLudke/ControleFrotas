...
router.get("/", autenticarToken, async (req, res) => {
  try {
    ...
    const take = limit
      ? Math.min(100, parseInt(String(limit), 10) || 100)
      : undefined;
...
router.get("/:id", autenticarToken, async (req, res) => {
  ...
  try {
    ...
    const isAdmin = req.user && req.user.role === "ADMIN";
    if (!isAdmin && m.userId !== req.user.id)
  ...
  try {
    ...
    if (!veiculoId || !data || !quilometragem || !tipo || !descricao) {
      return res.status(400).json({
        error:
          "Campos obrigatórios: veiculoId, data, quilometragem, tipo, descricao.",
      });
    }
...
router.patch("/:id", autenticarToken, async (req, res) => {
  ...
  try {
    ...
    if (custo !== undefined)
      updates.custo = custo !== null ? Number(custo) : null;
