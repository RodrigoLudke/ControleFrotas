...
const Motoristas = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response
          .json()
          .catch(() => ({ error: "Erro ao buscar motoristas." }));
        toast({
  ...
  const handleDeleteDriver = async (id: number) => {
    const ok = window.confirm(
      "Tem certeza que deseja deletar este motorista? Esta ação é irreversível.",
    );
    if (!ok) return;
    ...
    try {
      ...
      if (res.ok) {
        ...
        const err = await res
          .json()
          .catch(() => ({ error: "Erro ao deletar motorista." }));
        toast({
      ...
      toast({
        title: "Erro de conexão",
        description:
          "Não foi possível conectar ao servidor para deletar o motorista.",
        variant: "destructive",
      });
    } finally {
      setDeletingId(null);
                ...
                <TableBody>
                  {filteredDrivers.map((driver) => (
                    <TableRow key={driver.id} className="hover:bg-muted/50">
                      ...
                      <TableCell>
                        {getCategoryBadge(driver.categoria)}
                      </TableCell>
                      <TableCell>
