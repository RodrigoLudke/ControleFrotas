...
export default function VehicleDetails() {
  ...
  useEffect(() => {
    ...
    const normalizeManut = (m: any): Manutencao => {
      return {
        ...
        data: m.data ?? null,
        quilometragem:
          m.quilometragem !== undefined ? Number(m.quilometragem) : undefined,
        custo:
    ...
    const tryTripsWithFallback = async (vehFromApi?: any) => {
      ...
      if (vehFromApi) {
        ...
        for (const arr of maybeArrays) {
          if (Array.isArray(arr) && arr.length > 0) {
            console.debug(
              "VehicleDetails: usando viagens embutidas em /veiculos/:id",
            );
      ...
      try {
        ...
        console.debug(
          "VehicleDetails: tentativa /viagens?veiculoId ->",
        ...
        if (Array.isArray(arr) && arr.length > 0) {
          const normalized = arr
            .map(normalizeTrip)
            .filter((t) => Number(t.veiculoId) === vehicleId);
          if (normalized.length > 0) {
            console.debug(
              "VehicleDetails: /viagens?veiculoId retornou itens filtrados localmente ->",
              normalized.length,
            );
            return normalized;
          }
      ...
      try {
        ...
        if (resAdmin && resAdmin.ok) {
          ...
          const filtered = (all || [])
            ...
            .filter((t) => Number(t.veiculoId) === vehicleId);
          console.debug(
            "VehicleDetails: /viagens/admin -> total:",
            (all || []).length,
            "filtradas:",
            filtered.length,
          );
          ...
          console.debug(
            "VehicleDetails: /viagens/admin não autorizado (status)",
            resAdmin?.status,
          );
        }
      } catch (e) {
      ...
      try {
        ...
        const filtered = (arr || [])
          ...
          .filter((t) => Number(t.veiculoId) === vehicleId);
        console.debug(
          "VehicleDetails: fallback /viagens (user) -> total:",
          (arr || []).length,
          "filtradas:",
          filtered.length,
        );
    ...
    const load = async () => {
      setLoading(true);
      try {
        const [vRes, driversRes] = await Promise.allSettled([
          apiFetch(`/veiculos/${vehicleId}`),
          apiFetch(`/motoristas`),
        ]);
        //eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        if (fuelsRes.status === "fulfilled") {
          ...
          if (mounted)
            setFuels(Array.isArray(fArr) ? fArr.map(normalizeFuel) : []);
        } else {
        ...
        if (manRes.status === "fulfilled") {
          ...
          if (mounted)
            setManuts(Array.isArray(mArr) ? mArr.map(normalizeManut) : []);
        } else {
  ...
  const proxima = useMemo(() => {
    ...
    const agendada = list.find(
      (m) => (m.status || "").toLowerCase() === "agendada",
    );
  ...
  const stats = useMemo(() => {
    ...
    const fuelCost = (fuels || []).reduce(
      (acc, f) => acc + (f && f.custoTotal ? Number(f.custoTotal) : 0),
      0,
    );
    const manutCost = (manuts || []).reduce(
      (acc, m) => acc + (m && m.custo ? Number(m.custo) : 0),
      0,
    );
    ...
    try {
      const validFuels = [...(fuels || [])]
        .filter(
          (f) => f && f.quilometragem !== undefined && f.litros !== undefined,
        )
        //eslint-disable-next-line @typescript-eslint/no-explicit-any
        .map((f) => ({
          quil: Number((f as any).quilometragem),
          litros: Number((f as any).litros),
        }))
      ...
      mediaCombustivel =
        totalLitros > 0 ? +(totalKm / totalLitros).toFixed(2) : 0;
    } catch (e) {
      mediaCombustivel = 0;
    }
  ...
  const costOverTime = useMemo(() => {
    ...
    const push = (
      ...
      try {
        ...
        if (!map[ym])
          map[ym] = { ym, mes: label, combustivel: 0, manutencao: 0, custo: 0 };
        map[ym][key] += amount;
        map[ym].custo += amount;
      } catch (e) {
        /* ignore */
    ...
    fuels.forEach((f) =>
      push(f.data, Number(f.custoTotal ?? 0), "combustivel"),
    );
    manuts.forEach((m) =>
      push(m.data ?? undefined, Number(m.custo ?? 0), "manutencao"),
    );
  ...
  const driversUsage = useMemo(() => {
    ...
    const arr = Object.entries(count)
      .map(([uid, qtd]) => {
        ...
        return {
          name: d?.nome ?? d?.email ?? `Motorista ${uid}`,
          value: qtd,
          trips: qtd,
        };
      })
    ...
    return (
      <AdminLayout title="Veículo">
        <div className="mx-auto max-w-4xl p-6">
          ID do Veículo não especificado ou inválido.
        </div>
      </AdminLayout>
    );
  }
      ...
      <AdminLayout title="Erro">
        <div className="space-y-6 p-6">
          ...
          <Card>
            ...
            <CardContent>
              <p>
                O veículo com ID {id} não foi encontrado ou não pôde ser
                carregado.
              </p>
            </CardContent>
          </Card>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Válido até:{" "}
                      {vehicle.validadeSeguro
                        ? format(
                            new Date(vehicle.validadeSeguro),
                            "dd/MM/yyyy",
                            { locale: ptBR },
                          )
                        : "—"}
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {stats.kmRodados.toLocaleString("pt-BR")}
              </div>
              <p className="text-xs text-muted-foreground flex items-center mt-1">
                <TrendingUp className="h-3 w-3 mr-1 text-success" />{" "}
                {Math.round(Math.random() * 10)}% vs mês anterior
                  ...
                  <div className="flex-1 space-y-1">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm text-muted-foreground">
                      ...
                      <div className="flex items-center">
                        ...
                        {m.quilometragem
                          ? m.quilometragem.toLocaleString("pt-BR")
                          : "—"}{" "}
                        km
                      </div>
                      <div className="flex items-center">
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-2">
                      <p className="font-medium">
                        {f.posto ?? f.combustivel ?? "Abastecimento"}
                      </p>
                      <p className="text-lg font-bold">
                        {formatCurrency(Number(f.custoTotal ?? 0))}
                      </p>
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm text-muted-foreground">
                      <div className="flex items-center">
                      ...
                      <div>
                        R$ {Number(f.valorPorLitro).toFixed(2) ?? "—"}/L
                      </div>
                      <div className="flex items-center">
