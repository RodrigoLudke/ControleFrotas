...
export default function Abastecimentos() {
  ...
  const [vehiclesPlateMap, setVehiclesPlateMap] = useState<
    Record<number, string>
  >({});
  const [vehiclesModelMap, setVehiclesModelMap] = useState<
    Record<number, string>
  >({});
  const [driversNameMap, setDriversNameMap] = useState<Record<number, string>>(
    {},
  );
  const [driversEmailMap, setDriversEmailMap] = useState<
    Record<number, string>
  >({});
  ...
  const fetchAbastecimentos = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const e = await res
          .json()
          .catch(() => ({ error: "Erro ao buscar abastecimentos" }));
        toast({
          title: "Erro ao carregar abastecimentos",
          description:
            e.error || "Não foi possível carregar os abastecimentos.",
          variant: "destructive",
        });
        setAbastecimentos([]);
        setLoading(false);
      ...
      const [veicRes, motorRes] = await Promise.allSettled([
        apiFetch("/veiculos"),
        apiFetch("/motoristas"),
      ]);
      ...
      if (veicRes.status === "fulfilled" && veicRes.value.ok) {
        try {
          ...
          if (Array.isArray(veics)) {
            ...
            veics.forEach((v: any) => {
              if (v && v.id !== undefined) {
                vPlateMap[Number(v.id)] =
                  v.placa ?? v.modelo ?? `Veículo ${v.id}`;
      ...
      if (Array.isArray(data)) {
        data.forEach((a) => {
          ...
          if (vid !== undefined && vid !== null) {
            if (a.veiculo?.placa && !vPlateMap[Number(vid)])
              vPlateMap[Number(vid)] = a.veiculo.placa;
            if (a.veiculo?.modelo && !vModelMap[Number(vid)])
              vModelMap[Number(vid)] = a.veiculo.modelo;
          }
          const uid = a.user?.id ?? a.userId;
          if (uid !== undefined && uid !== null) {
            if (a.user?.nome && !dNameMap[Number(uid)])
              dNameMap[Number(uid)] = a.user.nome;
            if (a.user?.email && !dEmailMap[Number(uid)])
              dEmailMap[Number(uid)] = a.user.email;
          }
        });
      }
      ...
      toast({
        ...
        description:
          "Não foi possível conectar ao servidor para carregar abastecimentos.",
        variant: "destructive",
      });
      setAbastecimentos([]);
    } finally {
  ...
  const handleDelete = async (id: number) => {
    const ok = window.confirm(
      "Tem certeza que deseja deletar este abastecimento? Essa ação é irreversível.",
    );
    if (!ok) return;
    ...
    try {
      ...
      if (res.ok) {
        ...
        const err = await res
          .json()
          .catch(() => ({ error: "Erro ao deletar abastecimento." }));
        toast({
      ...
      toast({
        title: "Erro de conexão",
        description:
          "Não foi possível conectar ao servidor para deletar o abastecimento.",
        variant: "destructive",
      });
    } finally {
      setDeletingId(null);
    ...
    <AdminLayout title="Abastecimentos">
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row gap-4 justify-between">
          ...
          <Button
            onClick={() => navigate("/registrarabastecimentos")}
                  ...
                  {loading ? (
                    ...
                    filtered.map((a) => {
                      ...
                      const plate =
                        (vid !== undefined &&
                          vid !== null &&
                          vehiclesPlateMap[Number(vid)]) ||
                        (a.veiculo && (a.veiculo.placa ?? a.veiculo.modelo)) ||
                        "—";
                      const model =
                        (vid !== undefined &&
                          vid !== null &&
                          vehiclesModelMap[Number(vid)]) ||
                        (a.veiculo && a.veiculo.modelo) ||
                        "—";
                      ...
                      const driverName =
                        (uid !== undefined &&
                          uid !== null &&
                          driversNameMap[Number(uid)]) ||
                        (a.user && (a.user.nome ?? a.user.email)) ||
                        "—";
                      const driverEmail =
                        (uid !== undefined &&
                          uid !== null &&
                          driversEmailMap[Number(uid)]) ||
                        (a.user && a.user.email) ||
                        "—";
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell>
                            {formatDateTime(a.data ?? null)}
                          </TableCell>

                          <TableCell>
                            {a.quilometragem != null
                              ? `${a.quilometragem.toLocaleString()} km`
                              : "—"}
                          </TableCell>

                          <TableCell>
                            {formatNumber(a.litros, {
