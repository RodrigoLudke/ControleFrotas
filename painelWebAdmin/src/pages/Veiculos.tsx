...
const Veiculos = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response
          .json()
          .catch(() => ({ error: "Erro ao buscar veículos." }));
        toast({
          title: "Erro ao carregar veículos",
          description:
            errorData.error || "Não foi possível carregar a lista de veículos.",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  const handleDelete = async (id: number) => {
    const ok = window.confirm(
      "Tem certeza que deseja deletar este veículo? Essa ação é irreversível.",
    );
    if (!ok) return;
    ...
    try {
      ...
      if (res.ok) {
        ...
        const err = await res
          .json()
          .catch(() => ({ error: "Erro ao deletar veículo." }));
        toast({
      ...
      toast({
        title: "Erro de conexão",
        description:
          "Não foi possível conectar ao servidor para deletar o veículo.",
        variant: "destructive",
      });
    } finally {
      setDeletingId(null);
                    ...
                    <TableRow key={vehicle.id} className="hover:bg-muted/50">
                      ...
                      <TableCell>
                        <div className="space-y-1">
                          ...
                          {getInsuranceBadge(
                            vehicle.seguro ??
                              (vehicle.validadeSeguro ? "ativo" : undefined),
                          )}
