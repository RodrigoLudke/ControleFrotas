                ...
                <Card className="shadow-form">
                    <CardHeader className="space-y-2">
                        <div className="flex items-center space-x-2">
                            ...
                            <CardTitle className="text-2xl">{id ? "Editar Veículo" : "Cadastrar Novo Veículo"}</CardTitle>
                        </div>
                    ...
                    <CardContent>
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Input id="plate" value={formData.plate || ""} onChange={(e) => handleInputChange("plate", e.target.value.toUpperCase())} className={errors.plate ? "border-destructive" : ""} placeholder="ABC1234" maxLength={8} />
                                        {errors.plate && <p className="text-sm text-destructive">{errors.plate}</p>}
                                    </div>

                                    <div className="space-y-2">
                                        <Label htmlFor="brand">Marca *</Label>
                                        <Input id="brand" value={formData.brand || ""} onChange={(e) => handleInputChange("brand", e.target.value)} className={errors.brand ? "border-destructive" : ""} placeholder="Ex: Toyota, Volkswagen" />
                                        {errors.brand && <p className="text-sm text-destructive">{errors.brand}</p>}
                                    </div>

                                    <div className="space-y-2">
                                        <Label htmlFor="model">Modelo *</Label>
                                        <Input id="model" value={formData.model || ""} onChange={(e) => handleInputChange("model", e.target.value)} className={errors.model ? "border-destructive" : ""} placeholder="Ex: Corolla, Gol" />
                                        {errors.model && <p className="text-sm text-destructive">{errors.model}</p>}
                                    </div>

                                    <div className="space-y-2">
                                        <Label htmlFor="year">Ano *</Label>
                                        <Input id="year" type="number" value={formData.year || ""} onChange={(e) => handleInputChange("year", e.target.value)} className={errors.year ? "border-destructive" : ""} placeholder="2023" />
                                        {errors.year && <p className="text-sm text-destructive">{errors.year}</p>}
                                    </div>

                                    <div className="space-y-2">
                                        <Label htmlFor="color">Cor *</Label>
                                        <Input id="color" value={formData.color || ""} onChange={(e) => handleInputChange("color", e.target.value)} className={errors.color ? "border-destructive" : ""} placeholder="Branco, Preto, Prata" />
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Select value={formData.fuel || ""} onValueChange={(value) => handleInputChange("fuel", value)}>
                                            <SelectTrigger className={errors.fuel ? "border-destructive" : ""}><SelectValue placeholder="Selecione o combustível" /></SelectTrigger>
                                            <SelectContent>{fuelOptions.map((option) => (<SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>))}</SelectContent>
                                        </Select>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Input id="chassis" value={formData.chassis || ""} onChange={(e) => handleInputChange("chassis", e.target.value.toUpperCase())} className={errors.chassis ? "border-destructive" : ""} placeholder="17 caracteres" maxLength={17} />
                                        {errors.chassis && <p className="text-sm text-destructive">{errors.chassis}</p>}
                                    </div>

                                    <div className="space-y-2">
                                        <Label htmlFor="renavam">RENAVAM *</Label>
                                        <Input id="renavam" value={formData.renavam || ""} onChange={(e) => handleInputChange("renavam", e.target.value)} className={errors.renavam ? "border-destructive" : ""} placeholder="123456789" />
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Input id="capacity" type="number" value={formData.capacity || ""} onChange={(e) => handleInputChange("capacity", e.target.value)} className={errors.capacity ? "border-destructive" : ""} placeholder="5" />
                                        {errors.capacity && <p className="text-sm text-destructive">{errors.capacity}</p>}
                                    </div>

                                    <div className="space-y-2">
                                        <Label htmlFor="mileage">Quilometragem Atual *</Label>
                                        <Input id="mileage" type="number" value={formData.mileage || ""} onChange={(e) => handleInputChange("mileage", e.target.value)} className={errors.mileage ? "border-destructive" : ""} placeholder="50000" />
                            ...
                            {/* Dados de Compra */}
                            <div className="space-y-4">
                                <h3 className="text-lg font-medium text-foreground border-b border-border pb-2">Dados de Compra</h3>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        <Label htmlFor="purchaseDate">Data de Compra *</Label>
                                        <Input id="purchaseDate" type="date" value={formData.purchaseDate || ""} onChange={(e) => handleInputChange("purchaseDate", e.target.value)} className={errors.purchaseDate ? "border-destructive" : ""} />
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Input id="purchaseValue" type="number" step="0.01" value={formData.purchaseValue || ""} onChange={(e) => handleInputChange("purchaseValue", e.target.value)} className={errors.purchaseValue ? "border-destructive" : ""} placeholder="50000.00" />
                                        {errors.purchaseValue && <p className="text-sm text-destructive">{errors.purchaseValue}</p>}
                                    </div>
                                </div>
                            </div>
                            ...
                            {/* Dados do Seguro */}
                            <div className="space-y-4">
                                <h3 className="text-lg font-medium text-foreground border-b border-border pb-2">Dados do Seguro (Opcional)</h3>
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div className="space-y-2">
                                        <Label htmlFor="insuranceCompany">Seguradora</Label>
                                        <Input id="insuranceCompany" value={formData.insuranceCompany || ""} onChange={(e) => handleInputChange("insuranceCompany", e.target.value)} className={errors.insuranceCompany ? "border-destructive" : ""} placeholder="Nome da seguradora" />
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Input id="insurancePolicy" value={formData.insurancePolicy || ""} onChange={(e) => handleInputChange("insurancePolicy", e.target.value)} className={errors.insurancePolicy ? "border-destructive" : ""} placeholder="123456789" />
                                        {errors.insurancePolicy && <p className="text-sm text-destructive">{errors.insurancePolicy}</p>}
                                    </div>
                                    <div className="space-y-2">
                                        <Label htmlFor="insuranceExpiry">Vencimento do Seguro</Label>
                                        <Input id="insuranceExpiry" type="date" value={formData.insuranceExpiry || ""} onChange={(e) => handleInputChange("insuranceExpiry", e.target.value)} className={errors.insuranceExpiry ? "border-destructive" : ""} />
                                        {errors.insuranceExpiry && <p className="text-sm text-destructive">{errors.insuranceExpiry}</p>}
                                    </div>
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="observations">Observações</Label>
                                    <Textarea id="observations" value={formData.observations || ""} onChange={(e) => handleInputChange("observations", e.target.value)} className={errors.observations ? "border-destructive" : ""} placeholder="Informações adicionais sobre o veículo" rows={3} />
                            ...
                            <div className="flex justify-end space-x-4 pt-6 border-t border-border">
                                <Button type="button" variant="secondary" onClick={() => navigate("/veiculos")}>Cancelar</Button>
                                <Button type="submit" disabled={isLoading} className="min-w-[120px]">
                                    {isLoading ? "Salvando..." : (<><Save className="mr-2 h-4 w-4" />{id ? "Atualizar" : "Cadastrar"}</>)}
                                </Button>
                            </div>
                        </form>
                    </CardContent>
