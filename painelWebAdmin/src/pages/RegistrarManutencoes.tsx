...
export default function RegisterMaintenance() {
  ...
  const incomingAlert: any =
    (location && (location as any).state && (location as any).state.alert) ||
    null;
  ...
  const [veiculos, setVeiculos] = useState<
    Array<{ id: number; placa?: string; modelo?: string }>
  >([]);
  const [motoristas, setMotoristas] = useState<
    Array<{ id: number; nome?: string; email?: string }>
  >([]);
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof ManutencaoForm, string>>
  >({});
  ...
  useEffect(() => {
    ...
    const fetchManutencao = async () => {
      ...
      try {
        ...
        const dateLocal = data.data
          ? new Date(data.data).toISOString().slice(0, 16)
          : "";
        ...
        setForm({
          ...
          custo:
            data.custo !== undefined && data.custo !== null
              ? Number(String(data.custo))
              : undefined,
          local: data.local ?? "",
  ...
  useEffect(() => {
    ...
    try {
      ...
      const rawVeiculoId =
        alert.veiculoId ?? alert.veiculo?.id ?? alert.veiculo?.veiculoId;
      const rawVeiculoPlaca =
        alert.veiculo?.placa ?? alert.placa ?? alert.veiculoPlaca;
      ...
      if (rawVeiculoId !== undefined && rawVeiculoId !== null) {
        veiculoMatch = veiculos.find(
          (v) => String(v.id) === String(rawVeiculoId),
        );
      }
      if (!veiculoMatch && rawVeiculoPlaca) {
        const placaNormalized = String(rawVeiculoPlaca).toLowerCase();
        veiculoMatch = veiculos.find(
          (v) => v.placa && String(v.placa).toLowerCase() === placaNormalized,
        );
      }
      const veiculoIdFromAlert = veiculoMatch
        ? Number(veiculoMatch.id)
        : rawVeiculoId !== undefined
          ? Number(rawVeiculoId)
          : undefined;
      ...
      const rawUserName =
        alert.user?.nome ?? alert.user?.name ?? alert.user?.nomeCompleto;
      ...
      if (
        ...
        motoristaMatch = motoristas.find(
          (m) => String(m.id) === String(rawUserId),
        );
      }
      if (!motoristaMatch && rawUserEmail && motoristas.length > 0) {
        const emailNorm = String(rawUserEmail).toLowerCase();
        motoristaMatch = motoristas.find(
          (m) => m.email && String(m.email).toLowerCase() === emailNorm,
        );
      }
      if (!motoristaMatch && rawUserName && motoristas.length > 0) {
        const nameNorm = String(rawUserName).toLowerCase();
        motoristaMatch = motoristas.find(
          (m) => m.nome && String(m.nome).toLowerCase() === nameNorm,
        );
      }
      const userIdFromAlert = motoristaMatch
        ? Number(motoristaMatch.id)
        : rawUserId !== undefined
          ? Number(rawUserId)
          : undefined;
      ...
      setForm((prev) => ({
        ...prev,
        veiculoId:
          veiculoIdFromAlert !== undefined
            ? veiculoIdFromAlert
            : prev.veiculoId,
        userId: userIdFromAlert !== undefined ? userIdFromAlert : prev.userId,
        // data: NÃO alterar; deixamos para o usuário selecionar
        quilometragem:
          quilometragemFromAlert !== undefined
            ? Number(quilometragemFromAlert)
            : prev.quilometragem,
        tipo: mappedTipo,
        descricao: descricaoFromAlert
          ? `${descricaoFromAlert}`
          : prev.descricao,
        status: mappedStatus,
      }));
  ...
  //eslint-disable-next-line @typescript-eslint/no-explicit-any
  const validateField = <K extends keyof ManutencaoForm>(
    key: K,
    value: any,
  ) => {
    try {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const payloadRaw: any = {
        veiculoId:
          form.veiculoId !== undefined ? Number(form.veiculoId) : undefined,
        userId: form.userId !== undefined ? Number(form.userId) : undefined,
        data: form.data ? new Date(form.data).toISOString() : undefined,
        quilometragem:
          form.quilometragem !== undefined
            ? Number(form.quilometragem)
            : undefined,
        tipo: form.tipo,
        descricao: form.descricao,
        custo:
          form.custo !== undefined && form.custo !== null
            ? Number(String(form.custo))
            : undefined,
        local: form.local,
      ...
      if (res.ok) {
        ...
        const err = await res
          .json()
          .catch(() => ({ error: "Erro ao salvar manutenção." }));
        toast({
          title: "Erro",
      ...
      if (err instanceof z.ZodError) {
        ...
        err.errors.forEach((e) => {
          if (e.path && e.path[0])
            fieldErrs[e.path[0] as keyof ManutencaoForm] = e.message;
        });
        setErrors(fieldErrs);
        toast({
        ...
        toast({
          ...
          description:
            "Não foi possível salvar a manutenção. Verifique o console.",
          variant: "destructive",
        });
      }
    } finally {
  ...
  if (loadingInit) {
    return (
      <AdminLayout
        title={isEdit ? "Carregando manutenção..." : "Cadastrar Manutenção"}
      >
          ...
          <CardHeader className="space-y-2">
            <div className="flex items-center space-x-2">
              ...
              <CardTitle className="text-2xl">
                {isEdit ? "Editar Manutenção" : "Registrar Nova Manutenção"}
              </CardTitle>
            </div>
            <CardDescription>
              {isEdit
                ? "Altere os dados da manutenção"
                : "Preencha os dados para registrar a manutenção do veículo"}
            </CardDescription>
          </CardHeader>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(v) =>
                        handleChange("veiculoId", v ? Number(v) : undefined)
                      }
                    >
                      <SelectTrigger
                        id="veiculo"
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={
                        form.userId !== undefined ? String(form.userId) : ""
                      }
                      onValueChange={(v) => {
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      className={
                        errors.quilometragem ? "border-destructive" : ""
                      }
                      placeholder="0"
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    {errors.custo && (
                      <p className="text-sm text-destructive">{errors.custo}</p>
                    )}
                  </div>
