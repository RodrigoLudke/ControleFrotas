...
export default function Dashboard() {
  ...
  const {
    data: motoristasComLocalizacao,
    isLoading: isLoadingMotoristasLocalizacao,
  ...
  const motoristasNoMapa =
    motoristasComLocalizacao?.filter((m) => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      try {
        ...
        const [resVeic, resMotor] = await Promise.allSettled([
          apiFetch("/veiculos"),
          apiFetch("/motoristas"),
        ]);
        ...
        if (resVeic.status === "fulfilled" && resVeic.value.ok) {
          try {
            ...
            vehiclesList.forEach((v: any) => {
              if (v && v.id !== undefined)
                vMap[Number(v.id)] = v.placa ?? v.modelo ?? `Veículo ${v.id}`;
            });
          } catch (e) {
        ...
        if (resMotor.status === "fulfilled" && resMotor.value.ok) {
          try {
            ...
            driversList.forEach((m: any) => {
              if (m && m.id !== undefined)
                dMap[Number(m.id)] = m.nome ?? m.email ?? `Motorista ${m.id}`;
            });
          } catch (e) {
        ...
        // contadores
        //eslint-disable-next-line @typescript-eslint/no-explicit-any
        setCountVehiclesActive(
          vehiclesList.filter(
            (v: any) => (v.status ?? "").toString() === "disponivel",
          ).length,
        );
  ...
  const quickActions = [
    {
      ...
      to: "/registrarveiculos",
    },
    {
      label: "Adicionar Motorista",
      icon: Users,
      variant: "secondary" as const,
      to: "/registrarmotoristas",
    },
    {
      label: "Relatórios",
      icon: TrendingUp,
      ...
      <div className="space-y-6">
        ...
        <Card className="shadow-card">
          <CardHeader>
            ...
            <CardDescription>
              Visão geral das últimas viagens registradas (limitado a 5)
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody>
                  {loading ? (
                    ...
                    viagens.map((item) => {
                      const driver =
                        (item.userId && driversMap[item.userId]) || "—";
                      const plate =
                        (item.veiculoId && vehiclesMap[item.veiculoId]) || "—";
                      return (
                        <tr
                          key={item.id}
                          className="border-b bg-card hover:bg-muted/30"
                        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="lg:col-span-2 shadow-card">
            <CardHeader>
              ...
              <CardDescription>
                Próximas manutenções com status <strong>AGENDADA</strong>
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {loadingManutencoes ? (
                  ...
                  <div className="text-center py-6 text-muted-foreground">
                    Nenhuma manutenção agendada nos próximos dias.
                  </div>
                ) : (
                  manutencoesUpcoming.map((m) => {
                    const vehicleLabel =
                      (m.veiculoId && vehiclesMap[m.veiculoId]) ||
                      `Veículo ${m.veiculoId}`;
