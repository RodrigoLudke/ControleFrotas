...
export default function Trips() {
  ...
  const fetchTrips = async () => {
    ...
    try {
      ...
      if (!resTrips.ok) {
        const err = await resTrips
          .json()
          .catch(() => ({ error: "Erro ao buscar viagens" }));
        toast({
      ...
      if (Array.isArray(tripsData)) {
        tripsData.forEach((t) => {
          ...
          if (vid !== undefined && vid !== null) {
            if (t.veiculo?.placa && !vPlateMap[Number(vid)])
              vPlateMap[Number(vid)] = t.veiculo.placa;
            if (t.veiculo?.modelo && !vModelMap[Number(vid)])
              vModelMap[Number(vid)] = t.veiculo.modelo;
          }
          const uid = t.user?.id ?? t.userId;
          if (uid !== undefined && uid !== null) {
            if (t.user?.nome && !dNameMap[Number(uid)])
              dNameMap[Number(uid)] = t.user.nome;
            if (t.user?.email && !dEmailMap[Number(uid)])
              dEmailMap[Number(uid)] = t.user.email;
          }
        });
      }
      ...
      toast({
        ...
        description:
          "Não foi possível conectar ao servidor para carregar viagens.",
        variant: "destructive",
      });
      setTrips([]);
    } finally {
  ...
  const filteredTrips = trips.filter((t) => {
    const plate =
      (t.veiculoId !== undefined &&
        t.veiculoId !== null &&
        vehiclesPlateMap[Number(t.veiculoId)]) ||
      (t.veiculo && (t.veiculo.placa || t.veiculo.modelo)) ||
      "";
    const driver =
      (t.userId !== undefined &&
        t.userId !== null &&
        driversNameMap[Number(t.userId)]) ||
  ...
  const handleDelete = async (id: number) => {
    const ok = window.confirm(
      "Tem certeza que deseja deletar esta viagem? A ação é irreversível.",
    );
    if (!ok) return;
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res
          .json()
          .catch(() => ({ error: "Erro ao deletar viagem." }));
        toast({
      ...
      toast({
        title: "Erro de conexão",
        description:
          "Não foi possível conectar ao servidor para deletar a viagem.",
        variant: "destructive",
      });
    } finally {
      setDeletingId(null);
    ...
    <AdminLayout title="Viagens">
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row gap-4 justify-between">
          ...
          <Button
            onClick={() => navigate("/registrarviagens")}
        ...
        <Card className="shadow-card">
          <CardHeader>
            ...
            <CardDescription>
              {loading
                ? "Carregando..."
                : `${filteredTrips.length} viagem(ns) encontrado(s)`}
            </CardDescription>
          </CardHeader>
                  ...
                  {loading ? (
                    ...
                    filteredTrips.map((trip) => {
                      ...
                      const plate =
                        ...
                        (trip.veiculo &&
                          (trip.veiculo.placa ?? trip.veiculo.modelo)) ||
                        "—";
                      const model =
                        (vid !== undefined &&
                          vid !== null &&
                        ...
                        >
                          ...
                          <TableCell>
                            <div className="flex items-center">
                              ...
                              {trip.kmFinal
                                ? `${trip.kmFinal.toLocaleString()} km`
                                : "N/A"}
                            </div>
                          </TableCell>
