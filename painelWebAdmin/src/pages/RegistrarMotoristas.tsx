...
export default function RegisterDriver() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setFormData({
            ...
            rg: data.rg ?? "",
            cnh: data.cnh ?? "",
            cnhValidity: data.validadeCnh
              ? new Date(data.validadeCnh).toISOString().slice(0, 10)
              : "",
            cnhCategories: Array.isArray(data.categoria)
              ? data.categoria
              : data.categoria
                ? [data.categoria]
                : [],
            ...
            birthDate: data.dataNascimento
              ...
              : "",
            hireDate: data.dataContratacao
              ? new Date(data.dataContratacao).toISOString().slice(0, 10)
              : "",
          ...
          const err = await res
            .json()
            .catch(() => ({ error: "Motorista não encontrado." }));
          toast({
  ...
  const handleCategoryToggle = (category: string) => {
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        setErrors((prev) => ({
          ...prev,
          cnhCategories: error.errors[0].message,
        }));
      }
    }
  };
    ...
    try {
      ...
      const driverData: any = {
        ...
        salario:
          validatedData.salary !== undefined
            ? parseFloat(String(validatedData.salary))
            : undefined,
      ...
      if (response.ok) {
        toast({
          ...
          description: id
            ? "Alterações salvas."
            : `${validatedData.name} foi adicionado ao sistema.`,
          variant: "default",
        });
        navigate("/motoristas");
      } else {
        const errorData = await response
          .json()
          .catch(() => ({ error: "Erro ao salvar motorista." }));
        toast({
          title: "Erro",
          description:
            errorData.error || "Não foi possível salvar o motorista.",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  if (loadingInit) {
    return (
      <AdminLayout
        title={id ? "Carregando motorista..." : "Cadastrar Motorista"}
      >
          ...
          <CardHeader className="space-y-2">
            <div className="flex items-center space-x-2">
              ...
              <CardTitle className="text-2xl">
                {id ? "Editar Motorista" : "Cadastrar Novo Motorista"}
              </CardTitle>
            </div>
            <CardDescription>
              {id
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="password">
                      {id ? "Alterar senha (opcional)" : "Senha *"}
                    </Label>
                    <Input
                      id="password"
                      type="password"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        handleInputChange("cnhValidity", e.target.value)
                      }
                  ...
                  <div className="space-y-2">
                    ...
                    <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
                      <PopoverTrigger asChild>
                        ...
                        >
                          <span
                            className={`truncate ${formData.cnhCategories && formData.cnhCategories.length > 0 ? "" : "text-muted-foreground font-normal"}`}
                          >
                            {formData.cnhCategories &&
                            formData.cnhCategories.length > 0
                          ...
                          {CNH_CATEGORIES.map((category) => (
                            ...
                            >
                              <Checkbox
                                ...
                                checked={formData.cnhCategories?.includes(
                                  category.value,
                                )}
                                onCheckedChange={() =>
