...
const Manutencoes = () => {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      if (resM.ok) {
        ...
        const err = await resM
          .json()
          .catch(() => ({ error: "Erro ao buscar manutenções." }));
        toast({
          title: "Erro",
  ...
  const handleDelete = async (id: number) => {
    const ok = window.confirm(
      "Tem certeza que deseja deletar esta manutenção? Essa ação é irreversível.",
    );
    if (!ok) return;
    ...
    try {
      ...
      if (res.ok) {
        ...
        const e = await res
          .json()
          .catch(() => ({ error: "Erro ao deletar manutenção." }));
        toast({
          title: "Erro",
          description: e.error || "Não foi possível deletar.",
          variant: "destructive",
        });
      }
    } catch (err) {
                  ...
                  {loading ? (
                    ...
                    filtered.map((m) => (
                      <TableRow key={m.id} className="hover:bg-muted/50">
                        ...
                        <TableCell>
                          {formatDateTime(m.data ?? m.createdAt)}
                        </TableCell>
